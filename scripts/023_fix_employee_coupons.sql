...
-- Verify: Log how many employees now have coupons
DO $$
DECLARE
  total_employees INT;
  employees_with_coupons INT;
BEGIN
  SELECT COUNT(*) INTO total_employees FROM profiles WHERE role = 'employee';
  SELECT COUNT(DISTINCT employee_id) INTO employees_with_coupons FROM employee_coupons;
  
  RAISE NOTICE 'Employee coupon status: % employees, % have coupons', total_employees, employees_with_coupons;
END $$;
